...
const GameStatsPage: React.FC = () => {
  ...
  const homeTeamScore = lineScores.find(
    (score) => score.teamId === lineScores[0].teamId && score.gameId === gameId
  )?.pts || 0;
  ...
  const BoxScoreTable = () => {
    // Determine the maximum number of overtimes played
    const maxOvertimes = Math.max(
    ...
    const overtimeColumns = () => {
      const overtimes = [];
      for (let i = 1; i <= maxOvertimes; i++) {
        overtimes.push(
          <th key={`ot${i}`} className="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
            OT{i}
          </th>
        );
      }
    ...
    const overtimeScores = (score: LineScore) => {
      ...
      for (let i = 1; i <= maxOvertimes; i++) {
        const ptsOt = score[`ptsOt${i}` as keyof LineScore];
        overtimePeriods.push(
          <td key={`otScore${i}`} className="px-4 py-3 text-center whitespace-nowrap text-sm text-gray-500">
            {ptsOt || 0}
          </td>
        );
      }
    ...
    return (
      <div className="bg-white p-4 rounded-lg shadow-lg border border-[#333333] mb-8" style={{ maxWidth: '800px', margin: '0 auto' }}>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {lineScores.map((score, index) => (
                <tr key={index}>
                  ...
                  <td className="px-4 py-3 text-center whitespace-nowrap text-sm font-bold text-gray-900">
                    {score.pts}
                  </td>
                </tr>
              ))}
  ...
  const PlayerStatsTable = ({ teamId }: { teamId: number }) => {
    const filteredPlayerStats = Array.isArray(playerBoxStats)
      ? playerBoxStats.filter(
          (stats) => stats.teamId === teamId && stats.gameId === removeLeadingZeros(gameId)
        )
      : [];
          ...
          <table className="min-w-[1200px] divide-y divide-gray-200 table-fixed">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredPlayerStats.map((player, index) => (
                <tr key={index}>
                  <td className="w-24 px-2 py-2 text-center whitespace-nowrap text-sm font-medium text-gray-900">{player.playerName}</td>
  ...
  const TeamStatsComponent = ({ teamId }: { teamId: number }) => {
    const teamName = teamStats.find((team) => team.teamId === teamId)?.teamName || '';
    const logoUrl = getTeamLogo(teamId, teamStats);
      ...
      <div className="flex justify-center mb-4" style={{ marginBottom: '20px' }}>
        <button
          className={`px-4 py-2 font-bold text-lg ${
            activeTab === 'stats' ? 'text-[#00BFA6] border-b-4 border-[#00BFA6]' : 'text-[#333333]'
          }`}
        ...
        <button
          className={`ml-8 px-4 py-2 font-bold text-lg ${
            activeTab === 'roster' ? 'text-[#00BFA6] border-b-4 border-[#00BFA6]' : 'text-[#333333]'
          }`}
