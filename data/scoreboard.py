...
CREATE TABLE IF NOT EXISTS game_header (
    ...
    home_tv_broadcaster_abbreviation VARCHAR,
    away_tv_broadcaster_abbreviation VARCHAR,
    live_period_time_bcast VARCHAR,
    arena_name VARCHAR,
    wh_status INTEGER,
    wnba_commissioner_flag VARCHAR
);
""")
...
def fetch_last_game_dates(season='2023-24', season_type='Regular Season'):
    game_log = LeagueGameLog(season=season, season_type_all_star=season_type)
    games_df = game_log.get_data_frames()[0]
    last_5_games_df = games_df.tail(29)
    game_dates = last_5_games_df['GAME_DATE'].unique()
    return game_dates

# Fetch the last game dates of the season
game_dates = fetch_last_game_dates()
...
for game_date in game_dates:
        ...
        for index, game in game_header_df.iterrows():
            cursor.execute("""
                INSERT INTO game_header (
                    ...
                    away_tv_broadcaster_abbreviation, live_period_time_bcast, arena_name, 
                    wh_status, wnba_commissioner_flag
                ) 
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s::boolean)
                ON CONFLICT (game_id) DO NOTHING
            """, (
                game['GAME_ID'], game['GAME_DATE_EST'], game['GAME_SEQUENCE'], game['GAME_STATUS_ID'], 
                game['GAME_STATUS_TEXT'], game['HOME_TEAM_ID'], game['VISITOR_TEAM_ID'], game['SEASON'], 
                game['LIVE_PERIOD'], game['LIVE_PC_TIME'], game['NATL_TV_BROADCASTER_ABBREVIATION'], 
                game['HOME_TV_BROADCASTER_ABBREVIATION'], game['AWAY_TV_BROADCASTER_ABBREVIATION'], 
                game['LIVE_PERIOD_TIME_BCAST'], game['ARENA_NAME'], game['WH_STATUS'], 
                game.get('WNBA_COMMISSIONER_FLAG', None)  # This should be a boolean
            ))
        conn.commit()
    ...
    try:
        # Insert line score data directly into the database
            ...
            try:
                cursor.execute("""
                    INSERT INTO line_score (
                        ...
                        team_city_name, team_name, team_wins_losses, pts_qtr1, pts_qtr2, 
                ...
                """, (
                    line['GAME_DATE_EST'], line['GAME_SEQUENCE'], line['GAME_ID'], line['TEAM_ID'],
                    line['TEAM_ABBREVIATION'], line['TEAM_CITY_NAME'], line['TEAM_NAME'], 
    ...
    try:
        ...
        for index, standing in east_conf_standings_df.iterrows():
            ...
            """, (
                ...
                standing['G'], standing['W'], standing['L'], round(standing['W_PCT'], 3), standing['HOME_RECORD'], standing['ROAD_RECORD']
            ))
        conn.commit()
    except Exception as e:
