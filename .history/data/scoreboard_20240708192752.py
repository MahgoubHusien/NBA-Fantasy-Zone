    ...
    try:
        ...
        for index, line in line_score_df.iterrows():
            try:
                ...
                values = (
                    pd.to_datetime(line['GAME_DATE_EST']), int(line['GAME_SEQUENCE']), line['GAME_ID'], int(line['TEAM_ID']),
                    line['TEAM_ABBREVIATION'], line['TEAM_CITY_NAME'], line['TEAM_NAME'], line['TEAM_WINS_LOSSES'], 
                    int(line.get('PTS_QTR1', 0)), int(line.get('PTS_QTR2', 0)), int(line.get('PTS_QTR3', 0)), int(line.get('PTS_QTR4', 0)), 
                    int(line.get('PTS_OT1', 0)), int(line.get('PTS_OT2', 0)), int(line.get('PTS_OT3', 0)), int(line.get('PTS_OT4', 0)), 
                    int(line.get('PTS_OT5', 0)), int(line.get('PTS_OT6', 0)), int(line.get('PTS_OT7', 0)), int(line.get('PTS_OT8', 0)), 
                    int(line.get('PTS_OT9', 0)), int(line.get('PTS_OT10', 0)), int(line['PTS']), round(float(line['FG_PCT']), 3), 
                    round(float(line['FT_PCT']), 3), round(float(line['FG3_PCT']), 3), int(line['AST']), int(line['REB']), int(line['TOV'])
                )
