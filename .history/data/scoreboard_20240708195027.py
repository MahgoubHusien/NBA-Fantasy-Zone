...
for game_date in game_dates:
    ...
    try:
        ...
        for index, line in line_score_df.iterrows():
            try:
                ...
                values = (
                    ...
                    f"{round(float(line['FG_PCT']) * 100, 2):.2f}%" if pd.notnull(line['FG_PCT']) else "0.00%", 
                    f"{round(float(line['FT_PCT']) * 100, 2):.2f}%" if pd.notnull(line['FT_PCT']) else "0.00%", 
                    f"{round(float(line['FG3_PCT']) * 100, 2):.2f}%" if pd.notnull(line['FG3_PCT']) else "0.00%", 
        ...
        print(f"Error processing line score data: {e}")
...
line_score_csv_path = 'line_score.csv'
line_score_df.to_csv(line_score_csv_path, index=False)

# Load line_score data from CSV into the database
try:
    with open(line_score_csv_path, 'r') as f:
        next(f)  # Skip the header row
        cursor.copy_expert("COPY line_score FROM STDIN WITH CSV HEADER", f)
    conn.commit()
except Exception as e:
    print(f"Error loading line score data from CSV: {e}")
    conn.rollback()
