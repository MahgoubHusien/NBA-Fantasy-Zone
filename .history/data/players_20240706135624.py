...
def check_and_create_tables(conn):
    ...
    with conn.cursor() as cur:
        ...
        conn.commit()
    print("Tables `common_player_info` and `current_player_stats` checked and created if not exists.")
...
def fetch_and_insert_player_info(player_id, conn):
    try:
        ...
        player_data.update({
            'PPG': player_stats_dict.get('PTS', 0.0) / games_played,
            'APG': player_stats_dict.get('AST', 0.0) / games_played,
            'RPG': player_stats_dict.get('REB', 0.0) / games_played,
            'SPG': player_stats_dict.get('STL', 0.0) / games_played,
            'TOPG': player_stats_dict.get('TOV', 0.0) / games_played,
            'BPG': player_stats_dict.get('BLK', 0.0) / games_played,
            'PFPG': player_stats_dict.get('PF', 0.0) / games_played
        })
