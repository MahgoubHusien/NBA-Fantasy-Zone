...
def fetch_and_insert_standings(conn):
    try:
        # Fetch standings
        standings = LeagueStandingsV3(season='2023-24').get_data_frames()[0]
        
        query_standings = """
        INSERT INTO standings (
            league_id, season_id, team_id, team_city, team_name, team_slug, conference, conference_record, playoff_rank, clinch_indicator, division, division_record, division_rank, wins, losses, win_pct, league_rank, record, home, road, l10, long_win_streak, long_loss_streak, current_streak, conference_games_back, clinched_conference_title, clinched_playoff_birth, eliminated_conference, points_pg, opp_points_pg, diff_points_pg
        ...
        with conn.cursor() as cur:
            for index, row in standings.iterrows():
                cur.execute(query_standings, (
                    row['LeagueID'], row['SeasonID'], row['TeamID'], row['TeamCity'], row['TeamName'], row['TeamSlug'], row['Conference'], row['ConferenceRecord'], row['PlayoffRank'], row['ClinchIndicator'], row['Division'], row['DivisionRecord'], row['DivisionRank'], row['WINS'], row['LOSSES'], row['WinPCT'], row['LeagueRank'], row['Record'], row['HOME'], row['ROAD'], row['L10'], row['LongWinStreak'], row['LongLossStreak'], row['CurrentStreak'], row['ConferenceGamesBack'], row['ClinchedConferenceTitle'], row['ClinchedPlayoffBirth'], row['EliminatedConference'], row['PointsPG'], row['OppPointsPG'], row['DiffPointsPG']
                ))
            conn.commit()
